...
import pe.contabilidad.rest.client.domain.MoraPensionList;
...
public class RestClientController {
	...
	public String getAll(Model model) {
		...
		HttpEntity<MoraPension> entity = new HttpEntity<MoraPension>(headers);

		// Send the request as GET
		try {
			ResponseEntity<MoraPensionList> result = 
					restTemplate.exchange("http://localhost:8080/ws-contabilidad/contabilidad/alumnos",
							HttpMethod.GET, entity, MoraPensionList.class);
			// Add to model
	...
	public String getAlumno(@RequestParam("dni") String dni, Model model) {
		...
		// Send the request as GET
		try {
			ResponseEntity<MoraPensionList> result = restTemplate.exchange("http://localhost:8080/ws-contabilidad/contabilidad/alumno/{dni}", HttpMethod.GET, entity, MoraPensionList.class, dni);
			// Add to model
			model.addAttribute("alumno", result.getBody().getData());
